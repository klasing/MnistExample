// Exercise_2_1_starter
// taken from:
// https://www.boost.org/doc/libs/1_64_0/libs/ptr_container/doc/examples.html#a-large-example

// First we select wich container to use.
...
using namespace std;
namespace ns_exercise_2_1_starter {
	// Then we define a small polymorphic class hierarchy.
	//***************************************************************************
	//                     animal
	//***************************************************************************
	...
	private:
		// ...but due to advance in genetics, we can clone them!
	...
	// the default is to call the default 'operator delete()'.

	extern const string muuuh;
	extern const string oiink;

	//***************************************************************************
	//                     cow
	//***************************************************************************
	class cow : public animal {
		virtual string do_speak() const {
			return muuuh;
		}
	...
	class farm {
		...
		barn_type barn;
		// an error type
		struct farm_trouble : public exception {};
	public:
		...
		auto_ptr<barn_type> sell_farm() {
			return barn.release();
		}

		// However, if things are good, we might buy somebody
		// else's farm :=)
		void buy_farm(auto_ptr<barn_type> other) {
			...
			barn.transfer(barn.end(), // insert new animals at the end 
				*other); // we want to transfer all animals, so we use the whole container as argument
	...
	//                     exercise_2_1_starter
	//***************************************************************************
	inline void exercise_2_1_starter() {

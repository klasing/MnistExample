...
//// to clients when requested via the aquireObject() method. When a client is
...
//// once each for the lifetime of the program, not once per aquisition and release.
//
...
//// It expects the user to release every aquired object exactly once.
//// It expects the user to avoid using any objects that he or she has released.
//
//template<typename T>
//class ObjectPool {
//public:
//	// Creates an object with chunkSize object.
...
//
//	// Returns the object to the pool. Clients must not use the object
...
//throw(invalid_argument, bad_alloc) {
//
//	OutputDebugString(L"<<construcotr>> ObjectPool()\n");
//
//	if (chunkSize == 0)
...
//
//	OutputDebugString(L".allocateChunk()\n");
//
...
//
//template<typename T>
//shared_ptr<T> ObjectPool<T>::aquireObject() {
//
//	OutputDebugString(L".aquireObject()\n");
//
//	if (mFreeList.empty())
//		allocateChunk();
//
...
//
//	OutputDebugString(L".releaseObject()\n");
//
//	mFreeList.push(obj);
//}